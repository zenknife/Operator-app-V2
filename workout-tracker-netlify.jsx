...
const WORKOUT_TYPES = [
  { id: "upper-press", label: "UPPER PRESS", short: "PRESS", color: "#9FCB7B", icon: "△", group: "gym" },
  { id: "lower", label: "LOWER", short: "LOWER", color: "#B98A52", icon: "▽", group: "gym" },
  { id: "upper-pump", label: "UPPER PUMP", short: "PUMP", color: "#7FA36B", icon: "▲", group: "gym" },
  { id: "total", label: "TOTAL", short: "TOTAL", color: "#A09070", icon: "◇", group: "gym" },
  { id: "bjj-gi", label: "BJJ GI", short: "GI", color: "#6BA8A0", icon: "◎", group: "bjj" },
  { id: "bjj-nogi", label: "BJJ NO-GI", short: "NO-GI", color: "#5B8E86", icon: "◉", group: "bjj" },
  { id: "boxing", label: "BOXING", short: "BOX", color: "#C4885A", icon: "◆", group: "boxing" },
  { id: "engine", label: "ENGINE", short: "ENG", color: "#6B9A8A", icon: "⬡", group: "engine" },
  { id: "mobility", label: "MOBILITY", short: "MOB", color: "#8A8E78", icon: "∞", group: "other" },
];
...
const STORAGE_KEYS = {
  workouts: "workout-data-v5",
  bodyweight: "bodyweight-data-v1",
};
...
export default function WorkoutTracker() {
  ...
  useEffect(() => {
    const workoutsData = safeReadStorage(STORAGE_KEYS.workouts, {});
    const bodyweightEntries = safeReadStorage(STORAGE_KEYS.bodyweight, []);

    if (workoutsData && typeof workoutsData === "object") setWorkouts(workoutsData);
    if (Array.isArray(bodyweightEntries)) setBodyweightData(bodyweightEntries);
    setLoading(false);
  }, []);

  const saveWorkouts = useCallback((data) => {
    setWorkouts(data);
    safeWriteStorage(STORAGE_KEYS.workouts, data);
  }, []);

  const saveBodyweight = useCallback((data) => {
    setBodyweightData(data);
    safeWriteStorage(STORAGE_KEYS.bodyweight, data);
  }, []);
  ...
  if (activeWorkout) {
    ...
    return (
      <div style={st.shell}>
        <button style={st.backBtn} onClick={() => setActiveWorkout(null)}>← BACK</button>
          ...
          {activeWorkout.exercises.map((ex, exIdx) => (
            <div key={ex.id} style={st.exCard}>
              <div style={st.exHead}>
                ...
                <button style={st.exDel} onClick={() => deleteExercise(exIdx)}>×</button>
              </div>
                ...
                <div key={si} style={{ ...st.setRow, background: set.done ? "rgba(127,163,107,0.06)" : "transparent" }}>
                  ...
                  <div style={{ display: "flex", gap: 3 }}>
                    <button style={{ ...st.chk, background: set.done ? "#7FA36B" : "transparent", borderColor: set.done ? "#7FA36B" : "#3A4A3D" }}
                      onClick={() => updateSet(exIdx, si, { done: !set.done })}>{set.done ? "✓" : ""}</button>
                    <button style={st.rmSet} onClick={() => removeSet(exIdx, si)}>−</button>
                  </div>
                </div>
              ))}
        ...
        <div style={st.actionRow}>
          <button style={{ ...st.completeBtn, background: activeWorkout.completed ? "#7FA36B" : "#5A7A50" }}
            ...
            {activeWorkout.completed ? "COMPLETED ✓" : "MARK COMPLETE"}
          </button>
                ...
                return (
                  <button key={w.id} style={{ ...st.loggedCard, borderLeftColor: t?.color }} onClick={() => setActiveWorkout(w)}>
                    <div style={st.loggedTop}>
                      ...
                      {w.completed && <span style={st.loggedCheck}>✓</span>}
                    </div>
                    ...
                    {dw.length > 0 ? (
                      <div style={st.weekChips}>{dw.map((w) => { const t = WORKOUT_TYPES.find((x) => x.id === w.type); return (
                        <span key={w.id} style={{ ...st.weekChip, borderColor: t?.color + "55", background: t?.color + "10" }}>
                          ...
                          {w.completed && <span style={{ color: "#9FCB7B", fontSize: 10, marginLeft: 2 }}>✓</span>}
                        </span>); })}</div>
            ...
            <div>
              ...
              {[...bodyweightData].reverse().slice(0, 15).map((e) => (
                <div key={e.date} style={st.bwRow}>
                  ...
                  <button style={st.bwRowDel} onClick={() => deleteBodyweight(e.date)}>×</button>
                </div>
              ))}
            </div>
          )}
